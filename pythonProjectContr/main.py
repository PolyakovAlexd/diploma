...
def store_data_in_blockchain(id, data):
    tx_hash = contract.functions.storeData(int(id), data).transact({
...
def read_and_store_data(file_path):
    with open(file_path, mode='r', encoding='cp1251') as file:
        ...
        for index, row in enumerate(reader):
            ...
            if len(row) == 20:  # Ensure the row has the correct number of columns
                id, *data = row
                try:
                    receipt = store_data_in_blockchain(id, ';'.join(data))
                    logging.info(f'Data stored in transaction: {receipt.transactionHash.hex()}')
                except ValueError as e:
                    logging.error(f"Ошибка обработки данных в строке {index}: {row}, {e}")
            else:
...
def visualize_risks(data):
    try:
        fig = px.scatter(
            data,
            x='Вероятность угрозы (%)',
            y='Финансовые потери за инцидент (USD)',
            color='Оценка риска',
            title='Анализ рисков',
            labels={'Оценка риска': 'Уровень риска'},
            hover_data=['Название угрозы']  # Добавляем названия угроз в всплывающие подсказки
        )
        ...
        print("Путь к визуализации:", scatter_path)
        return scatter_path
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        scatter_path = visualize_risks(data)
        print(f"Path to visualization: {scatter_path}")
        ...
        create_report(high_risk, [scatter_path, residuals_path])
        print("Отчет успешно создан.")

        # Блокчейн интеграция
        print("Интеграция с блокчейном...")
        read_and_store_data(file_path)
